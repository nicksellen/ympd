                ...
                ${state.queue
                  .map((track) => {
                    const isCurrentAndPlaying =
                      isPlaying && isCurrent(state, track);
                    ...
                    function getProgressPercentage() {
                      ...
                      const { elapsedTime, totalTime } = state.transport;
                      return Math.round((elapsedTime / totalTime) * 100);
                    }
                    return `
                        <li class="${classes.join(" ")}">
                            <span class="queue-entry-title">${escape(
                              track.title
                            )}</span>
                            <span class="queue-entry-album">${escape(
                              track.album
                            )}</span>
                            <span class="queue-entry-artist">${escape(
                              track.artist
                            )}</span>
                            ${
                              !isCurrentAndPlaying
                                ? `
                                <span class="queue-entry-duration">${formatSeconds(
                                  track.duration
                                )}</span>
                                <span class="queue-entry-remove"><button onclick="removeTrack(${
                                  track.id
                                })">remove</button></span>
                            `
                                : ""
...
function renderBrowseNav(state) {
  ...
  const parts =
    state.browse.prefix === "/" ? [] : state.browse.prefix.split("/");
        ...
        <div class="browse-entries">
            <ul>
                ${state.browse.entries
                  .filter(
                    (entry) =>
                      entry.type === "directory" || entry.type === "song"
                  )
                  .map((entry) => {
                    if (entry.type === "song") {
                      return `
                            <li>
                                <span class="browse-song">
                                    <span class="browse-song-title">${escape(
                                      entry.title
                                    )}</span>
                                    <span class="browse-song-duration">${formatSeconds(
                                      entry.duration
                                    )}</span>
                                    <span class="browse-song-add"><button onclick='addTrack(${encodeParam(
                                      entry.uri
                                    )})'>add</button></span>
                                </span>
                            </li>
...
function renderBrowse(state) {
  return `
        <div id="browse" class="browse">
            ...
            ${
              state.browse.pending
                ? renderPending("Fetching entries")
                : renderBrowseEntries(state)
            }
        </div>
    `;
}
